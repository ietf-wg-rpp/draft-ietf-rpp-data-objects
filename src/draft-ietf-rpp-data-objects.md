...
: A set of rules defining the valid labels for a registry, including the permitted repertoire, contextual rules, and, where applicable, variant mappings. Also historically known as IDN tables.
...
A String that represents a Fully Qualified Domain Name (FQDN) that conforms to the syntax described in [@!RFC1035] and includes the following additional restrictions:

* The trailing dot required when these names are stored in a DNS zone is implicit and MUST NOT be provided.
* Internationalized domain names MUST be represented in ASCII Compatible Encoding (ACE) using the A-label, defined in Section 2.3.2.1 of the Definitions document [RFC5890]
* Case-insensitive comparison MUST be used when comparing two FQDNs.
* All representations generated by the server are to be in lowercase.
...
An IDN Variant Label is an alternate label that an IDN Table or Label Generation Ruleset (LGR) considers equivalent to another label for registration purposes, typically because the two labels are visually, phonetically, or otherwise linguistically related within a script or language. Two or more domain names are variants of one another when the server, applying the applicable LGR/IDN Table, determines that their labels stand in this relationship.
...
<!-- what operations operate on all variants in a bundle? transfer?-->
<!-- do we want convenience operation to find all registered variants in a bundle or all possible allowed (not yet registered) variants according to the LGR/IDN Table?-->
...
* Unicode Name
  ...
  * Mutability: create-only
  * Data Type: String
  * Description: The fully qualified name of the domain using the Unicode character set.
  * Constraints:
    * The value MUST be a fully qualified domain name in Unicode Normalization Form C (NFC) as defined in [UNICODE.NFC].
    * The server MUST validate the Unicode Name using the procedure described in section 4.2 of [RFC5891].
    * Every code point in the value MUST be classified as a candidate for inclusion by the rules defined in [@!RFC5892].
    * If the value contains a code point from a script with Right-to-Left directionality, the value MUST satisfy the Bidi Rule defined in [@!RFC5893].
...
The server MUST validate the name using the procedure described in section 4.2 of [RFC5891]. If the validation of the IDN name failed because it contained a code point not available in the specified IDN table, the server MUST return an error indicating the invalid code point. If the name does not map to the provided Unicode Name (uName), the server MUST respond with an appropriate error indicating the mismatch.

#### IDN Variant Detection

When a create request is submitted for a name associated with an IDN Table (uTable), the server MUST use the Label Generation Ruleset (LGR) or IDN Table identified by uTable, to determine whether the requested name is an IDN variant of any domain name already registered in the same namespace.
